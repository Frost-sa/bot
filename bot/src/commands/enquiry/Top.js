...
module.exports = {
  ...
  async exec(message, args) {
    ...
    if (!role) {
      const voiceFilter = member => member.voiceTime.total + member.voiceTime.vip > 0;
      const textFilter = member => member.textPoints > 0;
      const inviteFilter = member => member.invites > 0;
      if (membersData.find(voiceFilter)) membersData = membersData.filter(voiceFilter);
      console.log(membersData.length)
      if (membersData.find(textFilter)) membersData = membersData.filter(textFilter);
      console.log(membersData.length)
      if (membersData.find(inviteFilter)) membersData = membersData.filter(inviteFilter);
      console.log(membersData.length)
    }
    let totalText = 0;
