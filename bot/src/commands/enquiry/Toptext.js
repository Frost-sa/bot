...
module.exports = {
  name: "top text",
  aliases: ["الشات", "chat", "الكتابي", "text", "topchat", "toptext"],
  guildOnly: true,
  async exec(message, args) {
    ...
    function getEmbed () {
      ...
      .addField(`متصدري الشات في ${role ? role.name : "السيرفر"}`, `**${membersData.sort((a, b) => b.textPoints - a.textPoints).slice(currentPage * 10, currentPage * 10 + 10).map((member, index) => `${currentPage * 10 + index + 1}. <@${member.id.split("-")[0]}> | \` ${getPerc(member)} \` ${currentPage == 0 ? { 0: "🥇", 1: "🥈", 2: "🥉"}[index] || "" : ""} `).join("\n")}**`)
    ...
    function getPerc (member) {
      const perc = `${member.textPoints} points`;
      return perc;
    }
  }
}